...
int checkCert(
	...
	__block OCSP_CERTID* cert_id_for_check = OCSP_cert_to_id(nullptr, (X509*)cert, issuer);
	unsigned char* der = 0;
	int len = i2d_OCSP_REQUEST(req, &der);
	
	// Extract expiration date BEFORE async block since cert pointer may be freed
	...
	NSDate *__block expirationDate = [formatter dateFromString:fullDateString];
	...
	NSURLSessionDataTask *task = [session dataTaskWithRequest:request
		...
		if (error) {
			...
			if (cert_id_for_check) OCSP_CERTID_free(cert_id_for_check);
			return;
		}
		...
		if (httpResponse.statusCode == 200 && data) {
			...
			const unsigned char *respBytes = (const unsigned char *)[data bytes];
			resp = d2i_OCSP_RESPONSE(NULL, &respBytes, data.length);
			
			if (!resp) {
				completionHandler(2, expirationDate, @"Failed to parse OCSP response");
				if (cert_id_for_check) OCSP_CERTID_free(cert_id_for_check);
				return;
			}
			...
			if (!basic) {
				completionHandler(2, expirationDate, @"Failed to get basic OCSP response");
			...
			if (OCSP_resp_find_status(basic, cert_id_for_check, &status, &reason, NULL, NULL, NULL)) {
				// status: 0 = V_OCSP_CERTSTATUS_GOOD, 1 = V_OCSP_CERTSTATUS_REVOKED
				completionHandler(status, expirationDate, nil);
			} else {
				// Could not find status for this cert - treat as unknown/error
				completionHandler(2, expirationDate, @"OCSP status not found for certificate");
			}
			
			OCSP_CERTID_free(cert_id_for_check);
			cert_id_for_check = NULL;
