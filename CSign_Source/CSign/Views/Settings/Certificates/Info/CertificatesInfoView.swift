...
struct CertificatesInfoView: View {
	...
	@State private var _isCheckingOCSP = false
	...
	var body: some View {
		...
		.onAppear {
			...
			_isCheckingOCSP = true
			Storage.shared.revokagedCertificate(for: cert) { 
				DispatchQueue.main.async { _isCheckingOCSP = false }
			}
		}
	}
}
...
extension CertificatesInfoView {
	...
	private func _infoSection(data: Certificate) -> some View {
		...
		Section {
			...
			if _isCheckingOCSP {
				LabeledContent(.localized("Trạng thái")) {
					ProgressView()
						.controlSize(.small)
				}
			} else {
				_info(.localized("Trạng thái"), description: cert.revoked ? .localized("Đã thu hồi") : .localized("Tốt"))
